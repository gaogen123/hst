...
# V2.cust_indicator  2021-6-6   新增 BARSLAST函数 SLOPE,FORCAST线性回归预测函数
...
# V3.cust_indicator  2021-12-19 新增 TOPRANGE,LOWRANGE一级函数
...
def EMA(S,N):             #指数移动平均,为了精度 S>4*N  EMA至少需要120周期     alpha=2/(span+cust_indicator)
    return pd.Series(S).ewm(span=N, adjust=False).mean().values     

def SMA(S, N, M=1):       #中国式的SMA,至少需要120周期才精确 (雪球180周期)    alpha=cust_indicator/(cust_indicator+com)
    return pd.Series(S).ewm(alpha=M/N,adjust=False).mean().values           #com=N-M/M

def WMA(S, N):            #通达信S序列的N日加权移动平均 Yn = (cust_indicator*X1+2*X2+3*X3+...+n*Xn)/(cust_indicator+2+3+...+Xn)
    return pd.Series(S).rolling(N).apply(lambda x:x[::-1].cumsum().sum()*2/N/(N+1),raw=True).values 

def DMA(S, A):            #求S的动态移动平均，A作平滑因子,必须 0<A<cust_indicator  (此为核心函数，非指标）
...
def BARSLAST(S):                       #上一次条件成立到当前的周期, BARSLAST(C/REF(C,cust_indicator)>=cust_indicator.cust_indicator) 上一次涨停到今天的天数
