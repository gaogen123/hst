...
def run_backtest(code):
    cerebro = bt.Cerebro()
    strats = cerebro.addstrategy(TestStrategy, code=code)
    ...
    print(f'最终投资组合价值: %.2f' % cerebro.broker.getvalue())
...
if __name__ == '__main__':
    ...
    stock_codes = stock_info_sh_name_code_df['证券代码'].tolist()
    final_results = {}

    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(run_backtest, code): code for code in stock_codes}
    ...
    # 计算平均收益率和平均胜率
    avg_roi = sum(result['roi'] for result in final_results.values()) / len(final_results)
    avg_win_rate = sum(result['win_rate'] for result in final_results.values()) / len(final_results)
    print(f"\n平均收益率: {avg_roi:.2f}%")
